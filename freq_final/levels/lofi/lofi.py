...
class lofiLevel (Level):
    ...
    def get_name (self):        return "LoFi's in Ibiza"
    def get_abbrev_name (self): return "LoFi's in Ibiza"
    def get_artist (self):      return "Lo Fidelity Allstars"
    def get_genre (self):       return "disco house"
    def get_tempo (self):       return "129"
    def get_description (self): return "The Lo Fidelity Allstars are five lovely lads from up north who produce quality dance music for indie kids and club heads alike. The Lo Fi's are back with a new single, \"Lofi's in Ibiza,\" the first from a new album due early 2002.  www.lofidelityallstars.co.uk"
    ...
    def ps2_hard_effect_volumes (self, core):
        if core == 0:
            return [40, 40]               # Core 0 has delay depth of 30
        else:
            return [30, 30]               # Core 1 has delay depth of 40
        
    # What is the delay time for the given core?  Only valid for echo and delay.
    # Scale is 0-127
    def ps2_hard_delay_time (self, core):
        if core == 0:
            return 90                     # Core 0 has delay time of 40
        else:
            return 40                     # Core 1 has delay time of 27
