...
class TimedFeedback:
    ...
    def play_feedback (self):
        ...
        # After playing the note, make sure that other notes don't play
...
class TimedFeedbackMgr:
    ...
    def change_feedback_note (self, fb_id, new_note, note_len):
        for fb in self.feedback:
            if fb_id == fb.id:
                fb.note = new_note
                fb.note_length = note_len
...
class TutorialLevel (Level):
    def __init__ (self):
        ...
        self.bound_track_1 = 0
        self.bound_track_2 = 0
    ...
    def on_track_select (self, track):
        global curr_track
        print ' on_track_select, natural ', track
        curr_track = track
    ...
    def constrain_to_tracks (self, track_bound_1, track_bound_2):
        def on_track_select (self, track):
            global curr_track
            debug_print (' Constrain: Selected a track')
            debug_print (track)
            curr_track = track
            debug_print (curr_track)
        ...
        debug_print (' Constrain to tracks....')
        # Make sure we know what track we're on
        #hx.input ('enable', 0, 'rotL')
        #hx.input ('enable', 0, 'rotR')
        self.bound_track_1 = track_bound_1
        self.bound_track_2 = track_bound_2
        self.on_track_select (curr_track)

    def release_track_constraint (self):
        print ' RELEASING TRACK CONSTRAINT'
        self.restore_attrs ()
