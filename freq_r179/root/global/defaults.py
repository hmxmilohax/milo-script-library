...
#
# $Header: j:\\cvs/Freq/run/Global/defaults.py,v 1.171 2001/09/26 15:01:31 guest Exp $
#
##############################################################################
...
solo_powerups = ['aut', 'mul']
multi_powerups = ['neu', 'crp', 'fre', 'aut', 'bum']

def get_random_powerup (bar, multi):
    set = (solo_powerups, multi_powerups)[multi]
    while 1:
        name = whrandom.choice(set)
        if multi and bar < 16 and name == 'neu':
            pass                        # no neutralizers early
        else:
...
bank_cycler = MetagameBankCycler(8)
...
def cheat_listeningmode() :
    pass

def cheat_debugmode() :
    pass

def cheat_practicemode() :
    pass

def cheat_autocatcher() :
    add_powerups_to_all('aut')

def cheat_freestyle() :
    if community == 'local' :
        add_powerups_to_all('fre')

def cheat_multiplier() :
    if community == 'solo' :
        add_powerups_to_all('mul')

def cheat_neutralizer() :
    if community == 'local' :
        add_powerups_to_all('neu')

def cheat_crippler() :
    if community == 'local' :
        add_powerups_to_all('crp')

def cheat_bumper() :
    if community == 'local' :
        add_powerups_to_all('bum')
...
# remove dev cheats before shipping
newcheats = {
    "listeningmode":      [cheat_listeningmode],
    "debugmode":          [cheat_debugmode],
    "practicemode":       [cheat_practicemode],
